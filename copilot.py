...
TOOLS_SPEC = [
    ...
    },
    {
        ...
        "description": ("مرکز سیگنال: سه منبع — شکایت، درخواست توسعه، تعامل CRM. "
                        "شمارش هر دسته، روند دو نیم‌سال، نگاشت به مشتری، اقتصاد نقص "
                        "محصول (هر اصلاح پروندهٔ چند مشتری را هم‌زمان می‌بندد) و «ارزش "
...
class Copilot:
    ...
    def _ask_deterministic(self, question: str) -> dict:
        ...
        if hasw("مرکز سیگنال", "روند شکایت", "درخواست توسعه", "مشکل تکراری",
                ...
                "اقدام معلق", "سیگنال ها", "نقص محصول", "حل مشکل محصول",
                "چند مشتری همزمان"):
            src = ("complaint" if hasw("شکایت", "کیفیت", "نقص", "محصول") else
